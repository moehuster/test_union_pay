...
def Str2Bcd2(string):
    if len(string) % 2 == 1:
        string = string.ljust(len(string) + 1, '0')
...
class Iso8583:
    ...
    def ParseTPDU(self, p):
        DataType = self.__IsoSpec.DataType('TPDU')

        if DataType == DT.BCD:
            self.__TPDU = Bcd2Str(self.__iso[p:p+5])
            p += 5
        elif DataType == DT.ASCII:
            self.__TPDU = self.__iso[p:p+10].decode('latin')
            p += 10

        return p

    def ParseHEADER(self, p):
        DataType = self.__IsoSpec.DataType('HEADER')

        if DataType == DT.BCD:
            self.__HEADER = Bcd2Str(self.__iso[p:p+6])
            p += 6
        elif DataType == DT.ASCII:
            self.__HEADER = self.__iso[p:p+12].decode('latin')
            p += 12
    ...
    def BuildTPDU(self):
        if(self.__IsoSpec.DataType('TPDU') == DT.BCD):
            self.__iso += Str2Bcd(self.__TPDU)
        elif(self.__IsoSpec.DataType('TPDU') == DT.ASCII):
            self.__iso += self.__TPDU.encode('latin')

    def BuildHEADER(self):
        if(self.__IsoSpec.DataType('HEADER') == DT.BCD):
            self.__iso += Str2Bcd(self.__HEADER)
        elif(self.__IsoSpec.DataType('HEADER') == DT.ASCII):
            self.__iso += self.__HEADER.encode('latin')
    ...
    def FieldData(self, field, Value = None):
        if(Value == None):
            ...
            if DataType == DT.BIN:
                Len //= 2
            if Len > MaxLength:
                raise ValueError('Value length larger than field maximum ({0})'
                                 .format(self.__IsoSpec.MaxLength(field)))
            self.Field(field, 1)
    ...
    def PrintMessage(self):
        ...
        print("HEADER: [{0}]".format(self.__HEADER))
